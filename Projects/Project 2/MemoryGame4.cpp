...
int MemoryGame::input()const
{
    ...
    do
    {
        ...
        if (value < 0 || value >= numSlots)
        {
            cout << "input is not in [0," << numSlots-1 << "]. Re-enter: " << endl;
        }
        
        if (bShown[value])
        {
            cout << "This card is already flipped. Re-Enter: " << endl;
        }
    }
    while (value < 0 || value >= numSlots || bShown[value]);
...
void MemoryGame::play()
{
    randomize();
    display();
    int pairsFound = 0, rounds = 0, first;
    ...
    while(pairsFound < numPairs)
    {
        ...
        {
            if (values[index] != "" && values[index] == values[first])
            {
                bShown[index] = true;
                pairsFound++;
            }
    ...
    cout << "Congratulations! Found out all pairs in " << rounds << " rounds" << endl;
}